...
UE.registerUI('selectCascade', function(editor, uiName) {
	var btn = new UE.ui.Button({
		...
		onclick : function() {
			...
			var elmIndex = 0;
			$(selectedDom).closest("html").find("i-select").each(function(idx, el) {
				if (this == selectedDom.parentElement) {
					elmIndex = idx;
				}
			});
			...
			// 编辑模式
			var elm = $(selectedDom.parentElement);
			if (($(selectedDom.parentElement).attr("v-ab-cust-query") || "").indexOf("abCustQueryOpMap") == 0) {
				var data = {};
			...
			conf.ok = function(index, innerWindow) {
				...
				elm.html('<i-option v-for="item in ' + name + '" :value="item.' + data.opVal + '" :key="item.' + data.opVal + '">{{item.' + data.opTxt + '}}</i-option>');
	...
	editor.addListener('selectionchange', function() {
		...
		if (selectedDom.parentElement.localName == "i-select") {
