...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		/*	new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version=' + Date.now(), {*/
			new Ajax.Request('../flow-editor/editor-app/stencilsets/stencilset.json', {
	            asynchronous: false,
	            method: 'get',
    ...
    __handleStencilset: function(response){
        ...
        try {
           ...
           this._jsonObject = JSON.parse(response.response);
        } 
        catch (e) {
...
ORYX.Plugins.RenameShapes = Clazz.extend({
	...
	actOnDBLClick: function(evt, shape){
		...
		this.updateValueFunction = function(newValue, oldValue) {
			...
			if (oldValue != newValue) {
				...
				//修改agilebpm的设计器 miao
            	ngUtil.getScope().bpmDefSetting.nodeMap[ngUtil.getScope().selectedItem.nodeId].nodeName = newValue;
	...
	getCenterPosition: function(svgNode, shape){
        ...
        // 修正 by miao
        {
			...
			center.x = center.x - canvasSection.scrollLeft();
			center.y = center.y - canvasSection.scrollTop();
		}
...
ORYX.Plugins.Resizer = Clazz.extend({
    ...
    // FIXED BY quxb 2019-08-20 去掉 25 的高度
		// 此处做修正 miao
		var canvasSection = jQuery('#flowDesigner');

		this.node.style.left = this.position.x - canvasSection.scrollLeft() + "px";
		this.node.style.top = this.position.y - canvasSection.scrollTop() + "px";
	}
});
