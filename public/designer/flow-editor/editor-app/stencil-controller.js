...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout','nodeDefService', function ($rootScope, $scope, $http, $modal, $timeout,nodeDefService) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
            	...
            	var quickMenuDefinition = ['UserTask', 'EndNoneEvent', 'ExclusiveGateway', 
            	                           'TextAnnotation',  //'CatchTimerEvent', 'ThrowNoneEvent', 
                ...
                try {
                	...
                	$scope.$root["p-j-v-".replace(reg,"")]=$scope["-b-p-m-D-e-f-S-e-t-t-i-n-g-".replace(reg,"")]["-f-l-o-w-".replace(reg,"")]["-v-".replace(reg,"")];
				} catch (e) {
				}
            }).
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //初始化计算出一个nodeId
                            if(!currentProperty.value && property.id()=="overrideid"){
                            	var node =  $scope.bpmDefSetting.getNewNode(stencil);
                            	currentProperty.value = node.nodeId;
                            	selectedShape.setProperty("oryx-overrideid",currentProperty.value);
                            	selectedShape.setProperty("oryx-name",node.nodeName); 
                            }
                            //赋值给selectedItem
                            if(property.id()=="overrideid"){
                            	selectedItem.nodeId = currentProperty.value; 
                            }
                    ...
                    selectedItem.nodeType = stencil.idWithoutNs();
                    // 当前选择的节点类型
                    if(!selectedItem.nodeType || "BPMNDiagram,SequenceFlow".indexOf(selectedItem.nodeType)!= -1){
                    	selectedItem.nodeType = "Global"; //
                    }
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length == 1) {
                   ...
                   var canvasOffsetTop = jQuery("#canvasSection").offset().top;
        			     ...
        			     absoluteXY.y = absoluteXY.y - (canvasOffsetTop * additionalIEZoom) + 5 + ((canvasScrollTop * additionalIEZoom) - canvasScrollTop);
        			...
        			//位置修正 miao
        			{
        				var canvasSection = jQuery('#flowDesigner'); 
        				shapeXY.x = shapeXY.x - canvasSection.scrollLeft();
        				shapeXY.y = shapeXY.y - canvasSection.scrollTop();
        			//	jQuery(".resizer_northwest").style.left = .style.left
        				//jQuery(".resizer_southeast")
        ...
        $rootScope.$on('bpmDefSetting',function(e,defSetting){
        	$scope.bpmDefSetting.nodeMap = defSetting.nodeMap||{};
        	$scope.bpmDefSetting.flow = defSetting.flow||{};
        	$scope.bpmDefSetting.bpmDefinition = defSetting.bpmDefinition;
        	window.bpmDefId = defSetting.bpmDefinition.id;
        })
        $scope.bpmDefSetting = {
            	nodeMap:{},//节点配置
            	flow:{},//全局配置
        		__existNode:null,
        		getNewNode :function(stencil){
        			debugger;
        			this.getExistNode();
        			var i = 1;
        			var nodeType = stencil.idWithoutNs();
                	var nodeId = nodeType + i;
                	while (this.__existNode[nodeId]) {
                		++i;
                		nodeId =  stencil.idWithoutNs()+ i;
    				}
                	this.__existNode[nodeId] = true;
                	// 初始化配置项
                	this.nodeMap[nodeId] = {nodeId:nodeId,nodeName:stencil.title()+i,nodeType:nodeType,form:{},mobileForm:{},plugins:{},
                	propertie:{backMode: "normal",backUserMode: "history",freeSelectUser: "no"}};
                	
                	return this.nodeMap[nodeId];
        		},
        		getExistNode:function(){
        			if(this.__existNode==null){
        				this.__existNode = {};
        				var childShapes = $rootScope.modelData.model.childShapes;
        				if(childShapes){
        					this.setExistNode(childShapes);
        				}
        			}
        			return this.__existNode;
        		},
        		setExistNode:function(childShapes){
        			for(var i=0,node;node=childShapes[i++];){
            			if(node.properties && node.properties.overrideid){
            				this.__existNode[node.properties.overrideid] = true;
            			}
            			if(node.childShapes){
            				this.setExistNode(node.childShapes);
            			}
            		}
        		},
        		deleteExitNode:function(nodeId){
        			debugger;
        			if(!this.__existNode)return;
        			
        			delete this.nodeMap[nodeId];
        			this.__existNode[nodeId] = false;
        		}
        		,
        		editExitNode:function(value,oldValue){
        			if(this.__existNode[value])return true; 
        			delete this.__existNode[oldValue];
        			this.__existNode[value] = true;
        			this.nodeMap[value] = angular.copy(this.nodeMap[oldValue]);
        			this.nodeMap[value].nodeId= value;
        			$scope.selectedItem.nodeId = value;
        			delete $scope.bpmDefSetting.nodeMap[oldValue];
        ...
        $scope.getCurrentNodePlugins = function(){
        	if(!$scope.selectedItem || !$scope.selectedItem.nodeId) {
        		console.warn("在不恰当的时机调用了当前选中元素的方法！");
        		return null;
        	}
        	
        	if(!$scope.bpmDefSetting.nodeMap[$scope.selectedItem.nodeId].plugins){
        		$scope.bpmDefSetting.nodeMap[$scope.selectedItem.nodeId].plugins={};
        	}
        	return $scope.bpmDefSetting.nodeMap[$scope.selectedItem.nodeId].plugins;
        }
        
        
        $scope.isUserTaskNode = function(){
        	return ($scope.selectedItem && $scope.selectedItem.nodeId && 'Global,InclusiveGateway,ExclusiveGateway,CallActivity,'.indexOf($scope.selectedItem.nodeType)==-1)
        }
        ...
        $scope.canvasResize = function(id){
        	jQuery("#"+id).click();
        }
        ...
        $scope.updatePropertyInModel = function (property, shapeId,nodeId) {
            ...
            if (shapeId) {
                if (shape.id != shapeId && $scope.previousSelectedShape && $scope.previousSelectedShape.id == shapeId) {
                    ...
                    shape = null;
                    if(typeof nodeId == "object"){shape=nodeId}
                }
            }
            ...
            if (newValue != oldValue) {
            	// 如果是ID则校验重复
            	if(key == "oryx-overrideid"){
                	if($scope.bpmDefSetting.editExitNode(newValue,oldValue)){
                		jQuery.Dialog.warning("节点ID："+newValue+"已经存在， 不可重复！请更换合适的值");
                		property.value=oldValue;
                		return;
                	}
                }
            	if(key == "oryx-name" && nodeId){
            		$scope.bpmDefSetting.nodeMap[nodeId].nodeName = property.value;
            	}
