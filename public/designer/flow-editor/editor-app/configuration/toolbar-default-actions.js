...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    $scope.save = function (successCallback) {
        ...
        modelMetaData.name = $scope.bpmDefSetting.bpmDefinition.name;
        modelMetaData.description = $scope.bpmDefSetting.bpmDefinition.desc;
...
debugger;
        ...
        var params = {
            json_xml: json,
            svg_xml: svgDOM,
            name: $scope.bpmDefSetting.bpmDefinition.name, 
            rev:  $scope.bpmDefSetting.bpmDefinition.rev,
            description: $scope.bpmDefSetting.bpmDefinition.desc, 
            bpmDefSetting:JSON.stringify($scope.bpmDefSetting) // 流程属性扩展
        };
        var refreshAndCloseParent = false;
        //标记为发布并保存
        if(successCallback === true){
        	params.publish = true;
        	refreshAndCloseParent = true;
        	successCallback = "";
        }
        ...
        $http({    method: 'PUT',
            ...
            .success(function (data, status, headers, config) {
            	if(data && data.isOk===false){
                     ...
                     console.log('Something went wrong when updating the process model:' + data.msg);
 ...
 debugger;
				 ...
				 // 第一次完成设计 刷新当前页
				 if($scope.bpmDefSetting.bpmDefinition.rev ==1){
					 window.location.reload();
				 }
			 
                $scope.bpmDefSetting.bpmDefinition.rev =  $scope.bpmDefSetting.bpmDefinition.rev+1;
                ...
                $scope.status.loading = false;
                // 刷新父页面列表
            	if(window.opener && window.opener.reloadGrid) {window.opener.reloadGrid()}
            	
            	//发布新版本必须关闭 当前窗口
            	if(refreshAndCloseParent){
	            	try{
	            		jQuery.Dialog.close();
	            	}catch(e){
