...
dialogHelper__.initDialogs = function(){
	...
	jQuery.extend({
      ...
      var tf = top.frames[0];
      tf.layer.alert(content,conf,function(index){
				if(fn && jQuery.isFunction(fn))fn();
				if(tf && tf.layer){
          tf.layer.close(index);
				} 
				layer.close(index);
		...
		open:function(conf){
			...
			var openWindow = conf.topOpen ? top.frames[0] : window;
			...
			if(url){
				...
				// FIXED BY quxb 2019-08-29 iframe高度设为 100%
				var iframe ='<iframe  src="'+url+'" id="'+iframeId+'" name="'+iframeId+'" style="height:100%;width:100%;border:none;"></iframe>';
				conf.content = iframe;
			}
			...
			// FIXED BY quxb 2019-08-27 layer.js 和 layer.css 没有关于 btnAlign 属性的处理，需要手动设置
			if (conf.btnAlign) {
			  var btn_align = "right";
			  if (conf.btnAlign === "l") {
			    btn_align = "left";
        } else if (conf.btnAlign === "c") {
			    btn_align = "center";
        }

			  conf.success = function (layero) {
          layero.find('.layui-layer-btn').css('text-align', btn_align);
        }
      }
