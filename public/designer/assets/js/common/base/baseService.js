...
.directive('selectScript', ['$timeout', function ($timeout) {
  return {
    scope: {
      defId: "@defId"
    },
    link: function(scope, element, attrs, ctrl){
      var varTree;
      var codeMirrorBroadcast = attrs.broadcastString;
      scope.scriptTextWarning = attrs.text;
      scope.treeid = "varTree" + scope.$id;
      scope.tempTreeId = "tempTree" + scope.$id;

      // 替换 codeMirror 内容
      scope.insetCode = function (str) {
        scope.$parent.$broadcast(codeMirrorBroadcast, function (CodeMirror) {
      ...
      scope.initTree = function () {
        var param = {defId: scope.defId};
        if (attrs.hasOwnProperty("nodeId")) {
          param.nodeId = attrs.nodeId;
        }
        if (attrs.hasOwnProperty("includeBpmConstants")) {
          param.includeBpmConstants = true;
        }

        varTree = new ZtreeCreator(scope.tempTreeId, __ctx + "/bpm/processDef/variablesTree")
          .setDataKey({name: 'name'})
          .setCallback({onClick: scope.setVariable})
          .makeCombTree(scope.treeid, 300)
          .initZtree(param, 2);
      };

      // template中的内容渲染完成后，id已经有值，再初始化树
      $timeout(function () {
        scope.initTree();
      }, 0);

    },
    template: '<a class="btn btn-default" style="margin-bottom: 5px;" ng-click="selectScript()">常用脚本</a>\n' +
        '<a class="btn btn-default" style="margin: 0 5px 5px 0" id="{{treeid}}">流程变量</a>\n' +
        '<span class="fa fa-question-circle text-info" ng-show="scriptTextWarning" ab-tip title="{{scriptTextWarning}}"></span>\n' +
        '<div id="{{scope.tempTreeId}}"></div>'
  }
}])
 ...
 * 保存外部js对scope的修改。
 */
AngularUtil.isEmpty=function(obj){
