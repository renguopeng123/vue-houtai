			...
			element.on("click", function() {
				...
				rtn.then(function(data) {
					if (data.isOk) {
						...
						jQuery.Dialog.error(data.msg, data.cause);
					}
				}, function(errorCode) {
					jQuery.Dialog.error("请求失败!" + errorCode);
				});
			});
		}
	};
		...
		link : function(scope, element, attr, ctrl) {
			...
			rtn.then(function(result, status) {
				...
				if (result.isOk) {
				  ...
				  // FIXED by quxb 2019-08-23 增加【强制返回】属性，即使结果为空，也返回数据
					if (!attr.mustReturn && !result.data) {
						return;
					}
...
.directive('abEditor', function() {
	return {
        ...
        initialFrameWidth: "100%",      // FIXED BY quxb 2019-08-29 默认为100%，通过 html 设置外层宽度即可
...
/**
 * FIXED BY quxb 2019-08-10 实现数据的格式化，同时不破坏双向绑定
 *
 * @ab-boolean-checkbox 可为空， 格式以“/”分割 truevalue 和 falsevalue
 * 参照 ab-boolean 属性，用于复选框
 */
.directive('abBooleanCheckbox', function() {
  return {
    restrict : 'A',
    require : 'ngModel',
    scope : true,
    link : function(scope, element, attrs, ctrl) {
      var booleanConf = attrs.abBooleanCheckbox;
      if (booleanConf) {
        booleanConf = booleanConf.split("/");
      } else {
        booleanConf = [ true, false ];
