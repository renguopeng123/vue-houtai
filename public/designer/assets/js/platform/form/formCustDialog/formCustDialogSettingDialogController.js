...
app.controller('ctrl', [ '$scope', 'baseService', 'ArrayToolService', '$filter', function($scope, baseService, ArrayToolService, $filter) {
	...
	$scope.init = function() {
		...
		ToolsController.getEnum("cn.gwssi.ecloudbpm.base.api.query.QueryOP").then(function(data) {
			$scope.$apply(function() {
				$scope.QueryOP = data;
			});
		});

		ToolsController.getEnum("cn.gwssi.ecloudbpm.form.api.constant.FormCustDialogConditionFieldValueSource").then(function(data) {
			$scope.$apply(function() {
				$scope.FormCustDialogConditionFieldVALUESOURCE = data;
			});
		});
		
		ToolsController.getEnum("cn.gwssi.ecloudbpm.base.api.constant.Direction").then(function(data) {
			$scope.$apply(function() {
