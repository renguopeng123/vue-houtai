...
app.controller('ctrl', [ '$scope', 'baseService', 'ArrayToolService', '$filter', function($scope, baseService, ArrayToolService, $filter) {
	...
	$scope.init = function() {
		...
		ToolsController.getEnum("cn.gwssi.ecloudbpm.bus.api.constant.BusTableRelFkType").then(function(data) {
			$scope.$apply(function() {
				$scope.BusTableRelFkType = data;
			});
		});

		ToolsController.getEnum("cn.gwssi.ecloudbpm.bus.api.constant.BusinessObjectPersistenceType").then(function(data) {
			$scope.$apply(function() {
		...
		ToolsController.getInterFaceImpls("cn.gwssi.ecloudbpm.bus.api.service.IBusinessDataPersistenceBeanService").then(function(data) {
			$scope.$apply(function() {
