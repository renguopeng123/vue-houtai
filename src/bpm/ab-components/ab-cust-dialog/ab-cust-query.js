...
export default {
	...
	update:function(el,binding,vnode) {
		...
		let query = getQueryParam(el,binding,vnode);
		//值无变化，不操作
		...
		dialogData.timer = setTimeout(function(){
			let param = query;
			let url = "/form/formCustDialog/queryListData_" + vnode.data.attrs["dialog-key"];
...
function getQueryParam(el,binding,vnode){
	...
	let initParam = vnode.data.attrs["init-param"];
	if(initParam){
		for(let key in vnode.data.attrs.param){
...
function handleResult(el,binding,vnode,rows){
	...
	if(Array.isArray(value)){//赋值对象是数组
		...
		for(let key in dialogData.valueMap){
			let val = dialogData.valueMap[key];
			let list = [];
			data.forEach(item => {
				list.push(item[key]);
			});
